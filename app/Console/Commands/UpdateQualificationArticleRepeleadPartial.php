...
class UpdateQualificationArticleRepeleadPartial extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update-qualification-article-repelead-partial';
     ...
     * @var string
     */
    protected $description = 'Actualizacion de calificacion de leyes que esten derogadas parcialmente, se calificaran como no vigentes';
    ...
    public function handle()
    {
        try
        {
            ...
            $laws = Law::where('repealed', DB::raw("'Parcial'"))
            ...
            {
                $ids_articles = Article::select('id')
                ->where('law_id', $law->id)
                ->where('repealed', DB::raw("'SI'"))
                ->pluck('id')
                ->toArray();
