                              ...
                              <b-card-body border-variant="primary" class="mb-3 box-shadow-none">
                                <div class="rounded ui-bordered p-3 mb-3">
                                  <div v-if="file.apply_file == 'SI'">
                                    ...
                                    <b-form-row  v-if="file.required_expiration_date == 'SI' && document.class == 'Seguridad social'">
                                      <vue-advanced-select v-if="file.edit_document && auth.company_id != 159" class="col-md-6" v-model="file.month_pay" :disabled="false" name="month_pay" label="Mes de pago" placeholder="Seleccione el mes de pago" :options="months" :error="form.errorsFor(`activities.${index}.documents.${indexDocument}.files.${indexFile}.month_pay`)" :multiple="false" :allow-empty="false" help-text="El mes es opcional, de no seleccionar ninguno, se tomara el mes actual para realizar el calculo 77777.">
                                      </vue-advanced-select>
                                      <vue-datepicker v-if="auth.company_id != 159" :disabled="auth.company_id != 159" class="col-md-6" v-model="file.expirationDate" label="Fecha de vencimiento" :full-month-name="true" placeholder="Seleccione la fecha de vencimiento"  name="expirationDate" :disabled-dates="disabledDates" :error="form.errorsFor(`activities.${index}.documents.${indexDocument}.files.${indexFile}.expirationDate`)" help-text="La fecha de vencimiento se calcula con base en el mes de aporte que selecciones. Como el sistema es 'mes vencido', al elegir el período de octubre, se calculará tu fecha límite de pago en noviembre, según tu día de corte"/>
                                      <vue-datepicker v-else :disabled="false" class="col-md-6" v-model="file.expirationDate" label="Fecha de vencimiento" :full-month-name="true" placeholder="Seleccione la fecha de vencimiento"  name="expirationDate" :disabled-dates="disabledDates" :error="form.errorsFor(`activities.${index}.documents.${indexDocument}.files.${indexFile}.expirationDate`)" help-text="La fecha de vencimiento se calcula con base al número de dias configurado en tu información. Como el sistema es 'mes vencido', al estar en el mes de octubre por ejemplo, se calculará tu fecha límite de pago en noviembre, según tu día de corte"/>
                                    </b-form-row>
