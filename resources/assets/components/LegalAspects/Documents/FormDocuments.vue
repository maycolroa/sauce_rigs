        ...
        <b-col>
            <b-form :action="url" @submit.prevent="submit" autocomplete="off">
				<b-card border-variant="primary" class="mb-3 box-shadow-none">
                    ...
                    <b-form-row>
						<vue-file-simple v-if="isEdit || viewOnly" :help-text="`Para descargar el archivo actual, haga click <a href='/legalAspects/document/download/${this.$route.params.id}' target='blank'>aqui</a> `" :disabled="viewOnly" class="col-md-12" v-model="form.file" label="Archivo" name="file" :error="form.errorsFor('file')" placeholder="Seleccione un archivo" :maxFileSize="20"></vue-file-simple>
...
export default {
	...
	methods: {
		...
		this.form
			...
			.then(response => {
				this.loading = false;
				this.$router.push({ name: "legalaspects-documentslegals" });
			})
			.catch(error => {
				this.loading = false;
			});
