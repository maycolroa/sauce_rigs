<template>
  ...
  <b-form :action="url" @submit.prevent="submit" autocomplete="off">
    ...
    <b-form-row>
      ...
      <vue-ajax-advanced-select :disabled="viewOnly || !form.employee_regional_id" class="col-md-6" v-model="form.employee_headquarter_id" :error="form.errorsFor('employee_headquarter_id')" :selected-object="form.multiselect_sede" name="employee_headquarter_id" :label="keywordCheck('headquarter')" placeholder="Seleccione una opción" :url="headquartersDataUrl" :parameters="{regional: form.employee_regional_id, form: 'inspections'}" :emptyAll="empty.headquarter" @updateEmpty="updateEmptyKey('headquarter')" :multiple="true">
          </vue-ajax-advanced-select>
