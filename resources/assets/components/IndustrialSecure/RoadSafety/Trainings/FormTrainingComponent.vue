    ...
    <b-card no-body class="mb-2 border-secondary" style="width: 100%;">
      ...
      <b-collapse :id="`accordion-general`" visible :accordion="`accordion-master`">
        <b-card-body>
          ...
          <b-form-row>            
            <vue-ajax-advanced-select :disabled="viewOnly" class="col-md-6" v-model="form.position_id" :error="form.errorsFor('position_id')" :selected-object="form.multiselect_position" :multiple="true" :allowEmpty="true" name="position_id" :label="keywordCheck('positions')" placeholder="Seleccione las opciones a relacionar" :url="positionsDataUrl">
        ...
        <template v-for="(file, index) in form.files">
          <div :key="file.key">
              <b-form-row>
                ...
                <vue-file-simple v-if="file.type == 'Archivo'" :disabled="viewOnly" :help-text="file.id ? `Para descargar el archivo actual, haga click <a href='//industrialSecurity/roadsafety/training/download/${file.id}' target='blank'>aqui</a> ` : null" class="col-md-12" v-model="file.file" label="Archivo" name="file" placeholder="Seleccione un archivo" :error="form.errorsFor(`file`)" :maxFileSize="20"/>
  ...
  methods: {
    submit(e) {
      ...
      this.form
        ...
        .then(response => {
          this.loading = false;
          this.$router.push({ name: "industrialsecure-roadsafety-trainings" });
        })
        .catch(error => {
          this.loading = false;
        });
