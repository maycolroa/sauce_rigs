<template>
  ...
  <b-form :action="url" @submit.prevent="submit" autocomplete="off">
    ...
    <b-card no-body class="mb-2 border-secondary" style="width: 100%;">
      ...
      <b-collapse :id="`accordion-general`" visible :accordion="`accordion-master-1`">
        <b-card-body>
          <information-general
    ...
    <b-card v-if="form.add_fields && form.add_fields.length > 0" no-body class="mb-2 border-secondary" style="width: 100%;">
     <b-card-header class="bg-secondary">
      ...
      <b-collapse :id="`accordion-field`" visible :accordion="`accordion-master-2`">
        <b-card-body>
    ...
    <b-card no-body class="mb-2 border-secondary" style="width: 100%;">
      ...
      <b-collapse :id="`accordion-themes`" visible :accordion="`accordion-master-3`">
        <b-card-body>
          <div class="col-md-12">
              ...
              <perfect-scrollbar :options="{ wheelPropagation: true }" class="mb-4" style="height: 500px; padding-right: 15px; width: 100%;">
                <template v-for="(theme, index) in form.themes">
                  <b-card no-body class="mb-2 border-secondary" :key="theme.key" style="width: 100%;">
                    ...
                    <b-collapse :id="`accordion${theme.key}-1`" visible :accordion="`accordion-123${theme.key}`">
                      <b-card-body>
                        <b-form-row>
                                ...
                                <template v-for="(item, index2) in theme.items">
                                  <tr :key="index2">
                                    <td class="align-middle" v-if="!viewOnly">
                                      ...
                                      <b-modal :ref="`modalPlan${index}-${index2}`" :hideFooter="true" :id="`modals-default-${index+1}${index2}`" class="modal-top" size="lg">
                ...
                <div class="row float-right pt-10 pr-10" style="padding-top: 20px;">
                    <template>
                      <b-btn variant="primary" type="submit" :disabled="loading">Firmar</b-btn>
                    </template>
                </div>
              </perfect-scrollbar>
            </b-form-row>
...
import FormImage from '@/components/IndustrialSecure/DangerousConditions/FormImageComponent.vue';
  ...
  methods: {
    ...
    submit(e) {
      ...
      this.form
        ...
        .then(response => {
          this.loading = false;
          this.$router.push({ name: "dangerousconditions-inspections-request-firm" });
        })
        .catch(error => {
          this.loading = false;
        });
