      ...
      <template v-for="(element, index) in form.elements_id">
          <div :key="element.id">
            <b-form-row>
              ...
              <vue-input v-if="element.type == 'Identificable' && element.expired" :disabled="viewOnly" class="col-md-6" v-model="element.expiration_date" label="Máximos dias de uso" type="number" name="expiration_date" :error="form.errorsFor('expiration_date')" placeholder="Máximos dias de uso"></vue-input>
    ...
    typeElement(index)
    {
      ...
      axios.post('/industrialSecurity/epp/income/elementInfo', {id: this.form.elements_id[index].id_ele})
        .then(response => {
            ...
            this.form.elements_id[index].expired = response.data.expiration_date
        })
        .catch(error => {
