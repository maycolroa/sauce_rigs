  ...
  <b-form :action="url" @submit.prevent="submit" autocomplete="off">
    ...
    <b-card v-if="isEdit || isDeleted">
      <div>
        <template v-if="ready">
          <vue-table
            configName="industrialsecure-tags-possible-consequences-danger-search"
    ...
    <b-form-row v-if="isDeleted">
      ...
      <vue-ajax-advanced-select v-if="form.replace == 'SI'" :disabled="viewOnly" class="col-md-12" v-model="form.replace_deleted" name="replace_deleted" :error="form.errorsFor('replace_deleted')" label="Tags para remmplazar" placeholder="Seleccione la posible consecuencia del peligro" :url="tagsPossibleConsequencesDangerDataUrl" :multiple="false" :allowEmpty="true" :taggable="false">
    ...
    submit(e) {
      ...
      this.form
        ...
        .then(response => {
          ...
          this.$router.push({ name: "industrialsecure-dangermatrix-tags-possible-consequences-danger" });
        })
        .catch(error => {
          this.loading = false;
        });
