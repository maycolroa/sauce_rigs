<template>
  ...
  <b-form :action="url" @submit.prevent="submit" autocomplete="off">
    ...
    <b-form-row v-if="isDeleted">
      ...
      <vue-ajax-advanced-select v-if="form.replace == 'SI'" :disabled="viewOnly" class="col-md-12" v-model="form.replace_deleted" name="replace_deleted" :error="form.errorsFor('replace_deleted')" label="Tags para remmplazar" placeholder="Seleccione la descripcion del peligro" :url="tagsDangerDescriptionDataUrl" :multiple="false" :allowEmpty="true" :taggable="false">
    ...
    submit(e) {
      ...
      this.form
        ...
        .then(response => {
          ...
          this.$router.push({ name: "industrialsecure-dangermatrix-tags-danger-description" });
        })
        .catch(error => {
          this.loading = false;
        });
