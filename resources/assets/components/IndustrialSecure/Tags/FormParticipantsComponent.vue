  ...
  <b-form :action="url" @submit.prevent="submit" autocomplete="off">
    ...
    <b-card v-if="isEdit || isDeleted">
      <div>
        <template>
          <vue-table
            configName="industrialsecure-tags-participants-search"
    ...
    <b-form-row v-if="isDeleted">
      ...
      <vue-ajax-advanced-select v-if="form.replace == 'SI'" :disabled="viewOnly" class="col-md-12" v-model="form.replace_deleted" name="replace_deleted" :error="form.errorsFor('replace_deleted')" label="Tags para remmplazar" placeholder="Seleccione el participante" :url="tagsParticipantsDataUrl" :multiple="false" :allowEmpty="true" :taggable="false">
...
export default {
  ...
  data() {
    return {
      ...
      tagsParticipantsDataUrl: '/selects/tagsParticipants',
    };
  },
  methods: {
    submit(e) {
      ...
      this.form
        ...
        .then(response => {
          ...
          this.$router.push({ name: "industrialsecure-dangermatrix-tags-participants" });
        })
        .catch(error => {
          this.loading = false;
        });
