<template>
  <div>
    <header-module
      ...
      url="industrialsecure-roadsafety-customHelpers"
    />
      ...
      <b-card no-body>
        <b-card-body style="height: 1000px">
            <form-helper
                :help="data"
                :view-only="true"
                :cancel-url="{ name: 'industrialsecure-roadsafety-customHelpers'}"/>
        </b-card-body>
      </b-card>
    </div>
  </div>
...
export default {
  ...
  created(){
    axios.get(`/industrialSecurity/roadsafety/helpers/${this.$route.params.id}`)
    .then(response => {
