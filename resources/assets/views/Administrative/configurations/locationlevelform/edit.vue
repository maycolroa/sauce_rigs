<template>
  <div>
    <h4 class="font-weight-bold mb-4">
       <span class="text-muted font-weight-light">Nivel localización en formulario /</span> Editar
    </h4>
      ...
      <b-card no-body>
        <b-card-body>
            <form-location-level-form-componet
                :url="`/administration/configurations/locationLevelForms/${this.$route.params.id}`"
                method="PUT"
                :modules="modules"
                :locationLevels="locationLevels"
                :location-level-form="data"
                :is-edit="true"
...
export default {
  ...
  metaInfo: {
    title: 'Nivel localización en formulario - Editar'
  },
  components:{
  ...
  created(){
    axios.get(`/administration/configurations/locationLevelForms/${this.$route.params.id}`)
    .then(response => {
