...
<br/>
    ...
    @if ($check->start_recommendations)
        ...
        @if ($check->indefinite_recommendations != "NO")
            <p>Las anteriores recomendaciones han sido emitidas por {{$check->origin_recommendations}} y son <b>indefinidas</b> y emitidas por <b>{{$check->origin_recommendations}}</b>@if($recommendations), se deben tener en cuenta para actividades <b>{{ $recommendations }}</b>@endif.</p>
        @else
            <p>Las anteriores recomendaciones han sido emitidas por {{$check->origin_recommendations}} y son emitidas por <b>{{ $check->origin_recommendations }}</b>, @if($recommendations)se deben tener en cuenta para actividades <b>{{ $recommendations }}</b>,@endif de <b>carácter temporal</b> por {{$check->time_different}} días a partir del {{$check->start_recommendations}} hasta el {{$check->end_recommendations}}.</p>
        @endif
